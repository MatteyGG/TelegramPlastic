...
import {
  getDialogHistory,
  getDialogHistoryByUsername,
} from "../modules/dialogHistory";
...
const DEFAULT_PROMPT = {
  system_prompt:
    "Вы эксперт по 3D-печати на FDM принтерах. Отвечайте кратко, используя историю диалога.",
};
...
function getMainMenu(): InlineKeyboard {
  return new InlineKeyboard()
    ...
    .text("📝 Конфиги", "admin_configs")
    .row()
    .text("💬 Диалоги", "admin_dialogs")
    .text("👥 Админы", "admin_manage")
    .row()
    .text("📁 Логи", "admin_logs")
    .text("✨ Продукты", "admin_products")
    .row()
...
function getConfigMenu(): InlineKeyboard {
  return new InlineKeyboard()
    ...
    .text("Prompt", "config_prompts_1")
    .row()
    .text("◀️ Назад", "admin_main");
}

// Меню действий с конфигом
function getConfigItemMenu(
  type: string,
  key: string,
  page: number = 1
): InlineKeyboard {
  return new InlineKeyboard()
    ...
    .text("🗑️ Удалить", `delete_${type}_${key}`)
    .row()
...
function getResetConfirmMenu(type: string): InlineKeyboard {
  return new InlineKeyboard()
    ...
    .text(
      "❌ Нет",
      type === "response" ? "config_responses_1" : "config_prompts_1"
    );
}
...
function getDialogsMenu(): InlineKeyboard {
  return new InlineKeyboard()
    ...
    .text("По username", "dialog_by_username")
    .row()
    .text("◀️ Назад", "admin_main");
}
...
function getProductsMenu(): InlineKeyboard {
  return new InlineKeyboard()
    ...
    .text("📊 Статистика продуктов", "products_stats")
    .row()
    .text("◀️ Назад", "admin_main");
}
...
function validateProductsStructure(data: any): {
  isValid: boolean;
  errors?: string[];
} {
  const errors: string[] = [];
...
function getAdminsMenu(): InlineKeyboard {
  return new InlineKeyboard()
    ...
    .text("➕ Добавить админа", "add_admin")
    .row()
    .text("➖ Убрать админа", "remove_admin")
    .row()
    .text("◀️ Назад", "admin_main");
}
...
async function handleResponsesConfig(ctx: any, page: number) {
  ...
  responses.forEach((item: { key: any; value: string }, index: number) => {
    responseText += `<b>${skip + index + 1}. ${
      item.key
    }:</b>\n<code>${item.value.substring(0, 50)}${
  ...
  responses.forEach((item: { key: any }) => {
...
async function handlePromptsConfig(ctx: any, page: number) {
  ...
  prompts.forEach((item: { key: any; value: string }, index: number) => {
    promptText += `<b>${skip + index + 1}. ${
      item.key
    }:</b>\n<code>${item.value.substring(0, 100)}${
...
- Продуктов: ${productCount}
  ...
  bot.callbackQuery("products_stats", async (ctx) => {
    ...
    materials.forEach((material: { material: any; _count: { _all: any } }) => {
...
<code>${prompt.value}</code>
  ...
  bot.callbackQuery("add_response", async (ctx) => {
    ...
    await ctx.editMessageText(
      "➕ <b>Добавление нового response</b>\n\nВведите данные в формате:\n<code>ключ|значение</code>\n\nНапример:\n<code>start|Привет! Я помогу выбрать материалы для 3D-печати. Задавайте вопросы! 🛠\n startAdmin|👋Добро пожаловать в панель управления!</code>",
      { parse_mode: "HTML" }
    );
    await ctx.answerCallbackQuery();
  });
  ...
  bot.callbackQuery("reset_responses_confirm", async (ctx) => {
    ...
    try {
      ...
      await ctx.answerCallbackQuery({
        text: "✅ Responses сброшены к значениям по умолчанию",
      });
  ...
  bot.callbackQuery("reset_prompt_confirm", async (ctx) => {
    ...
    try {
      ...
      await ctx.answerCallbackQuery({
        text: "✅ Prompts сброшены к значениям по умолчанию",
      });
  ...
  bot.callbackQuery("dialog_by_username", async (ctx) => {
    ...
    await ctx.editMessageText(
      "Введите username для просмотра истории (без @):"
    );
    await ctx.answerCallbackQuery();
    ...
    admins.forEach(
      (
        admin: {
          userId: any;
          username: any;
          createdAt: { toLocaleDateString: (arg0: string) => any };
        },
        index: number
      ) => {
        ...
        adminsText += `\n   Добавлен: ${admin.createdAt.toLocaleDateString(
          "ru-RU"
        )}\n\n`;
      }
    );
  ...
  bot.callbackQuery("add_admin", async (ctx) => {
    ...
    await ctx.editMessageText(
      "Введите ID пользователя для добавления в админы (можно с @username):"
    );
    await ctx.answerCallbackQuery();
  ...
  bot.callbackQuery("remove_admin", async (ctx) => {
    ...
    await ctx.editMessageText(
      "Введите ID пользователя для удаления из админов (можно с @username):"
    );
    await ctx.answerCallbackQuery();
    ...
    if (state.currentPage === "awaiting_chat_id") {
            ...
            if (record.products) {
              try {
                ...
                if (products.length > 0) {
                  historyText += `   <b>📦 Продукты:</b> ${products
                    .map((p: any) => p.title)
                    .join(", ")}\n`;
                }
              } catch (e) {
      ...
      try {
        ...
        if (userId.startsWith("@")) {
          ...
          await ctx.reply(
            "❌ Необходимо ввести User ID, а не username. Username может меняться."
          );
          return;
        }
  ...
  bot.on("message:document", async (ctx) => {
    ...
    try {
      ...
      if ((document.file_size ?? 0) > 5 * 1024 * 1024) {
        // 5MB лимит
